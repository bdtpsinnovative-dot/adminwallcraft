...
export default function ManageCompaniesPage() {
  ...
  // 🔥 ฟังก์ชันใหม่: ดึงข้อมูลทั้งหมดแบบ Loop ทะลุ Limit 1000
  const handleExportAll = async () => {
    ...
    try {
      ...
      // วนลูปดึงทีละ 1000 จนกว่าข้อมูลจะหมด
      while (hasMore) {
        ...
        if (data && data.length > 0) {
          ...
          // ถ้าดึงมาได้น้อยกว่า step แสดงว่าหมดตูดแล้ว
      ...
      // จัด Format ให้ตรงกับ Template นำเข้า (Customer, Companies)
      const formattedData = allData.map(c => ({
        customer: c.name,
        companies: c.customer_types?.name || ''
      }));

      // แปลงเป็น CSV ด้วย papaparse
  ...
  const handleBulkDelete = async () => {
    ...
    else { setSelectedIds([]); fetchCompanies(); }
    setLoading(false);
  };
                ...
                companies.map((comp) => (
                  <tr key={comp.id} className={`hover:bg-indigo-50/30 transition-colors group ${selectedIds.includes(comp.id) ? 'bg-indigo-50/50' : ''}`}>
                    ...
                    <td className="px-6 py-5 text-right">
                      <button onClick={() => { if(confirm('ลบรายการนี้?')) { supabase.from('companies').delete().eq('id', comp.id).then(() => fetchCompanies()) } }} className="p-2 text-slate-300 hover:text-rose-500 transition-all"><Trash2 size={18} /></button>
                    </td>
                  </tr>
                ))
              )}
